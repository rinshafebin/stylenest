...
const AdminHome = () => {
  ...
  const sidebarItems = [
    ...
    { icon: Package, label: 'Products', path: '/allproduct', count: '2.4k' },
            ...
            {sidebarItems.map((item, idx) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <item.icon
                    className={`w-5 h-5 ${
                      item.active ? 'text-rose-600' : 'text-gray-400 group-hover:text-black'
                    }`}
                  />
          ...
          <main className="flex-1 overflow-y-auto p-6 bg-gray-50">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {statsCards.map((card, i) => (
                <div
                  key={i}
                  className="bg-white rounded-2xl p-5 border border-gray-100 shadow hover:shadow-lg transition"
                >
                  <div className="flex items-center justify-between mb-4">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-r ${card.gradient} text-white shadow`}
                    >
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span
                        className={`text-sm font-medium ${
                          card.trend === 'up' ? 'text-rose-600' : 'text-red-600'
                        }`}
                      >
                        {card.change}
                      </span>
